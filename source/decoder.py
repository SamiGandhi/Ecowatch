...
def build_received_video_roi(frame_number):
    ...
    zero_frame = np.zeros((util.para.default_height, util.para.default_width), dtype=np.uint8)
    ones_frame = np.ones((util.para.default_height, util.para.default_width), dtype=np.uint8)
    ones_block = np.ones((8, 8), dtype=np.uint8)
    image_name, mask_name, enhanced_image_name = "", "", ""
    packet_blocks = []
    with open(os.path.join(util.para.trace_file_path, 'st-packet'), 'r') as trace_file:
            ...
            if line and line[0] != '#' and line[0] != ' ':
                ...
                if frame_type == "M":
                    if decoded_frame_vector[frame_nb - 2].frame_type == "N":
                        ...
                        all_blocks = [0, util.para.default_width * util.para.default_height // 64 - 1]
                        for empty_layer in empty_layers:
                            fill_in_main_frame_roi(decoded_frame_vector[frame_nb - 2], all_blocks, empty_layer, util.para.level_numbers,ber)

                    if util.para.interleaving == "torus":
                        fill_in_main_frame_torus(decoded_frame_vector[frame_nb - 2], packet_blocks, layer_nb, util.para.level_numbers, util.videoP.frame_height // 8, util.videoP.frame_width // 8, util.simP.trace_path,signal_loss)
                    else:
                        fill_in_main_frame_roi(decoded_frame_vector[frame_nb - 2], packet_blocks, layer_nb, util.para.level_numbers,ber)
    ...
    for ind, decoded_frame_struct in enumerate(decoded_frame_vector):
        ...
        if decoded_frame_struct.frame_type == "M":            
            ...
            print('\n')
            '''
            captured_frame = get_image(os.path.join(util.para.roi_frame_dir, f"roi_frame{ind + 2}.png"))
            ...
            decoded_frame_struct.PSNR = util.getPSNR(decoded_frame_struct.decoded_frame, orig_frame)
            decoded_frame_struct.SSIM = util.getSSIM(decoded_frame_struct.decoded_frame, orig_frame)
            util.write_decoded_frame_record(ind + 1, decoded_frame_struct)

            image_name = os.path.join(util.para.decoded_frames_dir, f"frame{ind + 2}.png")
...
def build_received_video(frame_number):
            ...
            if line and line[0] != '#' and line[0] != ' ':
                ...
                if frame_type == "M":
                    if decoded_frame_vector[frame_nb - 1].frame_type == "N":
                        ...
                        all_blocks = [0, util.para.default_width * util.para.default_height // 64 - 1]
                        for empty_layer in empty_layers:
                            fill_in_main_frame(decoded_frame_vector[frame_nb - 1], all_blocks, empty_layer, util.para.level_numbers,ber)

                    if util.para.interleaving == "torus":
                        fill_in_main_frame_torus(decoded_frame_vector[frame_nb - 1], packet_blocks, layer_nb, util.para.level_numbers, util.videoP.frame_height // 8, util.videoP.frame_width // 8, util.simP.trace_path,signal_loss)
                    else:
                        fill_in_main_frame(decoded_frame_vector[frame_nb - 1], packet_blocks, layer_nb, util.para.level_numbers,ber)
    ...
    for ind, decoded_frame_struct in enumerate(decoded_frame_vector):
        ...
        if decoded_frame_struct.frame_type == "M":
            for i in range(0, util.para.default_height, 8):
                for j in range(0, util.para.default_width, 8):
                    ...
                    if np.sum(tbloc) < 255 * 64:
                        tbloc[:] = 0
            captured_frame = get_image(os.path.join(util.para.captured_frame_dir, f"frame{ind + 1}.png"))
        ...
        decoded_frame_struct.SSIM = util.getSSIM(decoded_frame_struct.decoded_frame, orig_frame)
        util.write_decoded_frame_record(ind + 1, decoded_frame_struct)
        image_name = os.path.join(util.para.decoded_frames_dir, f"frame{ind + 1}.png")
        cv2.imwrite(image_name, decoded_frame_struct.decoded_frame)
